...
def getDisparityMap(model, transform, img_path, device):
    img = Image.open(img_path)
    img = np.array(img)  # Convert to NumPy array 
    input_batch = transform(img).to(device)  # Move the input batch to the device (CPU/GPU)
    with torch.no_grad():
...
def main():
    ...
    if imgP.is_file():
        disp = getDisparityMap(midas, midas_transform, imgP, device)
        disp[disp < 0] = 0
        disp = disp + 1e-3
