...
class SnowEffectGenerator:
    def __init__(self):
        ...
        self._weather2visibility = (1000, 2500)
    ...
    def genEffect(self, img_path: str, depth_path: str):
        print(f"Loading image: {img_path}, and depth map: {depth_path}")
        I = np.array(Image.open(img_path))
        D = np.load(depth_path)
        
        print(f"Image shape: {I.shape}, Depth map shape: {D.shape}")
        
        hI, wI, _ = I.shape
        hD, wD = D.shape
        
        if hI != hD or wI != wD:
            print(f"Rescaling depth map to match image size: ({hI}, {wI})")
        ...
        print(f"Illumination level: {illumination}")
        ...
        print(f"Generated snowy image with shape: {I_snow.shape}")
...
def main():
    ...
    if clearP.is_file() and (depthP.is_file() and depthP.suffix == ".npy"):
        print(f"Processing image: {clearP}, with depth map: {depthP}")
        snowy = snowgen.genEffect(clearP, depthP)
        if args.show:
            Image.fromarray(snowy).show()
        
    if clearP.is_dir() and depthP.is_dir():
        print(f"Found directories, processing images from: {clearP} and depth maps from: {depthP}")
        
        image_files = sorted(Path(clearP).glob("*.jpg"))  # Debug: force checking for .jpg files
        print(f"Found {len(image_files)} image files.")
        
        # Fix: Correct matching logic for depth maps (remove .jpg extension and match only once)
        depth_files = [Path(depthP) / (imgf.stem.split("-FRONT")[0] + "-FRONT.npy") for imgf in image_files]
        print(f"Expected depth map files: {depth_files}")
        
        valid_files = [idx for idx, f in enumerate(depth_files) if f.exists()]
        print(f"Valid pairs of images and depth maps: {valid_files}")
        
        image_files = [image_files[idx] for idx in valid_files] 
        depth_files = [depth_files[idx] for idx in valid_files]
        print(f"Final valid pairs: {list(zip(image_files, depth_files))}")
        ...
        for imgp, depthp in tqdm(zip(image_files, depth_files), total=len(image_files)):
            print(f"Processing image: {imgp}, saving output to: {save_folder / (imgp.stem + '-ssyn.jpg')}")
