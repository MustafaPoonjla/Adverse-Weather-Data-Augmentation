...
def parse_arguments():
    ...
    parser.add_argument("--save-folder", type=str, help="Path to save the generated images.", required=True)
    parser.add_argument("--vgg", type=str, help="Path to the pretrained VGG model.", required=True)

    parser.add_argument('--cuda', action='store_true', help="use cuda.")
    parser.add_argument('--ext', type=str, default="nst", help="extension for generated image.")  # Changed default
...
def main():
    ...
    # Update paths to point to your clear images folder
    content_images = sorted(Path(args.content_imgs).glob("*.jpg"))  # content images in images folder
    style_images = sorted(Path(args.style_imgs).glob("*"))  # style images in output folder
    ...
    for cimg in tqdm(content_images):
        ...
        simg = random.choice(style_images)  # Select a random snowy image
        ...
        # Save the output image with the '-nst' suffix
        output_img.save(save_folder / f"{name}-nst.{extension}")  # Added '-nst' to the filename
